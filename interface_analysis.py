...
Phase 2 features (PAE-aware, requires PKL data):
...
Usage (standalone):
    ...
    python interface_analysis.py --pdb structure.pdb --pkl result.pkl  # Phase 2
...
def compute_interface_geometry(contact_result: ContactResult) -> dict:
    ...
    Returns:
        Dictionary with keys:
            ...
            interface_symmetry: min(frac_a, frac_b) / max(frac_a, frac_b).
                                1.0 = perfectly symmetric; 0.0 = one-sided.
            contacts_per_interface_residue: Average contacts per interface
                                            residue (density measure).
...
def compute_interface_plddt(contact_result: ContactResult) -> dict:
    ...
    Returns:
        Dictionary with keys:
            ...
            interface_plddt_high_fraction: Fraction of interface residues
                                           with pLDDT >= 70.
    """
...
def extract_interface_pae(
    ...
    *,
    ...
    Args:
        ...
        chain_lengths: DEPRECATED for multi-chain use.  Tuple of
...
def compute_interface_pae_features(
    contact_result: ContactResult,
    pae_matrix: np.ndarray,
    chain_lengths: Optional[tuple[int, int]] = None,
    *,
    ...
    *relative positioning* between residue pairs, not just per-residue confidence.
    ...
    interface_pae = extract_interface_pae(
...
def identify_confident_interface_residues(
    ...
    *,
    ...
    Args:
        ...
        pae_matrix: Full PAE matrix from the PKL file.
        chain_lengths: Tuple of (n_residues_chain_A, n_residues_chain_B).
                       Legacy parameter; use chain_offsets for multi-chain.
        chain_residue_numbers: Optional dict of chain_id -> list of PDB residue
                               numbers, for mapping back to biological numbering.
    ...
    # Filter: PAE < threshold AND both residues have pLDDT >= threshold
    confident_mask = (
...
def analyse_interface(
    ...
    # Phase 4: Composite score (None without PAE, but included for consistency)
    result['interface_confidence_score'] = compute_interface_confidence(result)

    return result


def analyse_interface_with_pae(
    pdb_path: Union[str, Path],
    pae_matrix: np.ndarray,
    chain_lengths: Optional[tuple[int, int]] = None,
    threshold: float = DEFAULT_CONTACT_THRESHOLD,
    ...
    # Phase 2: Confident residues
    confident = identify_confident_interface_residues(
        contact_result, pae_matrix, chain_lengths=None,
        chain_residue_numbers=chain_info.chain_res_numbers,
        chain_offsets=pae_chain_offsets,
        cb_to_ca_maps=cb_maps,
    )
...
def analyse_interface_from_contact_result(
    contact_result: ContactResult,
...
def compute_extended_flags(
    interface_features: dict,
...
def main() -> None:
    ...
    parser.add_argument("--threshold", type=float, default=DEFAULT_CONTACT_THRESHOLD,
                        help=f"Contact distance threshold in Å (default: {DEFAULT_CONTACT_THRESHOLD})")
